...
for i = 1:length(Pout)
    ...
    % Boundary conditions
    equations{3, 1} = iPout == conj(I1).*V1;
...
for i = 1:length(Pout)
    ...
    % Boundary conditions
    equations{3, 1} = iPout == real(conj(I1).*V1);
    equations{4, 1} = Vgrid == 33000/sqrt(3);
    %equations{5, 1} = imag(Igrid) == 0;
    %equations{6, 1} = imag(Vgrid) == 0;
    equations{5, 1} = imag(conj(Igrid)*Vgrid) == 0;
...
Qgrid5 = imag(SgridVals5);
Ploss5 = Ztot.*abs(I1Vals5(2, :)).^2;
...
grid on
xlabel('RPM','interpreter','latex','FontSize',14)
ylabel('$$\Delta P_{Loss}$$','interpreter','latex','FontSize',14)
